...
function tierLabel(t) {
  return { great:'Great', good:'Good', ok:'Fair',
...
function refreshHero() {
  const byTier = { great: 0, 'mid-hi': 0, 'mid-lo': 0, low: 0 };
...
function buildTable() {
  ...
  FORMULAS.forEach((latex, idx) => {
    ...
    tr.dataset.tier = (score === null || score < 0.3) ? 'low'
      : score >= 0.8 ? 'great'
      : score >= 0.5 ? 'mid-hi'
      : 'mid-lo';
